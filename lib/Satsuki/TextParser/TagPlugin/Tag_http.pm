...
sub http {
	...
	if ($ary->[0] eq 'image' && $tags->{"$cmd:image"} && $tags->{'&image'}) {
		...
		$cmd .= ':image';
		$tag  = $tags->{$cmd};
		my $image = $parser_obj->load_tag('&image');
		return &$image($parser_obj, $tag, $cmd, $ary);
	}
	# 属性/リンク名
	...
	my $name = $parser_obj->make_name($ary, $url);
...
sub mailto {
	...
	my $name = $parser_obj->make_name($ary, $mail);
