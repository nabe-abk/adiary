...
////////////////////////////////////////////////////////////////////////////////
$$.dom_init( function($R){
	const $tbl = $R.findx('div:not(.table-container) table');
	for(const t of $tbl) {
		const $div = $('<div>').addClass('table-container');
		$(t).replaceWith($div);
		$div.append(t);
	}
});
...
// fancybox
//##############################################################################
$(document).on('click', 'a[data-lightbox]', function(evt){
...
$.fancybox.defaults.lang = 'ja';
$.fancybox.defaults.i18n.ja = {
        CLOSE: "閉じる",
        NEXT: "次の画像",
        PREV: "前の画像",
        ERROR: "画像のロードに失敗しました。",
